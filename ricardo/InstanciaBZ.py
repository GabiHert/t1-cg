...
class InstanciaBZ:   
    ...
    def AtualizaPosicao(self, tempo_decorrido):
        if self.curva_atual is None:
            self.curva_atual = pontos_curva  # Atribui uma curva aleatória no início
            self.comprimento_curva = Bezier.calculaComprimentoDaCurva(self.curva_atual)  # Calcula o comprimento da curva
    ...
    # Atualize o parâmetro t para mover o personagem ao longo da curva
        ...
        if self.t >= 0.5:  # Se t atingir 0.5, alteramos para a próxima curva
            self.curva_proxima = f.CurvaAleatoria(pontos_curvas)

        if self.t > 1:  # Se t for maior que 1, o personagem chegou ao fim da curva
            self.t = 0  # Reseta t para o início da curva
            self.curva_atual = self.curva_proxima
            self.curva_proxima = f.CurvaAleatoria(pontos_curvas)
            self.comprimento_curva = f.calculaComprimentoDaCurva(self.curva_atual)  # Recalcula o comprimento para a nova curva
     # Atualiza a posição e a rotação do personagem
